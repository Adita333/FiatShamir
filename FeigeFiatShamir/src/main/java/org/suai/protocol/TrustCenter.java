...
/**
 * El centro de confianza T publica un gran número n=pq, donde p y q son números primos que se mantienen en secreto.
 * También se eligen números enteros k y t como parámetros de seguridad.
 */
public class TrustCenter {
    private static final Logger logger = Logger.getLogger(TrustCenter.class.getName());

    private static final int P_LENGTH = 128; // Longitud de p
    private static final int Q_LENGTH = 32;   // Longitud de q
    private static final int SEED_LENGTH = 128; // Longitud de la semilla
    ...
    private BigInteger[] generatePrimes(int pLength, int qLength, int seedLength) throws NoSuchAlgorithmException {
        if (seedLength < qLength) {
            throw new IllegalArgumentException("Error: la longitud de la semilla es menor que la longitud de q.");
        }
        int outLength = 128;
