...
public class Bob extends TrustCenter {
    ...
    public boolean checkZ(BigInteger y, BigInteger[] x) {
        // Asegurarse de que x tenga la longitud esperada
        if (x.length != k) {
            throw new IllegalArgumentException("El tamaño del arreglo x no coincide con k.");
        }
        ...
        out.println("z: " + z);
        out.println("x: " + x[0]);
        out.println("-x: " + (x.length > 1 ? x[1] : null)); // Manejo seguro para x[1]
        out.println();

        return z.compareTo(x[0]) == 0 || (x.length > 1 && z.compareTo(x[1]) == 0);
    }
}